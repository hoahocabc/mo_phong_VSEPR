...
function preload() {
  try {
    arialFont = loadFont(
      ...
      () => {},
      (err) => {
        console.warn("loadFont failed for Arial.ttf", err);
        arialFont = null;
      }
    );
  } catch (e) {
...
function sanitizeScalar(s, fallback = 0) {
  return isFiniteNumber(s) ? s : fallback;
}
...
/////////////////////// Smooth zoom helpers ///////////////////////
let targetZoom = 1.0;
let ZOOM_LERP = isMobileDevice ? 0.1 : 0.22;
let lastTouchDist = 0;
...
function alignZ_with_roll_fix(direction) {
  ...
  if (abs(th) < 1e-6) currY = createVector(0, 1, 0);
    ...
    if (rotAxis.mag() < 1e-6) currY = createVector(0, -1, 0);
      ...
      rotAxis.normalize();
      currY = rotateVectorRodrigues(createVector(0, 1, 0), rotAxis, th);
    }
  }
...
/////////////////////// Cylinder helper for bonds ///////////////////////
function drawCylinderBetween(p1, p2, radius = 4, detail = CYL_DETAIL) {
  let a = sanitizeVec(p1, createVector(0, 0, 0));
  let b = sanitizeVec(p2, createVector(0, 0, 0));
  let delta = p5.Vector.sub(b, a);
  let len = delta.mag();
  if (len < 1e-6) return;
  let dir = delta.copy().normalize();
  let mid = p5.Vector.add(a, b).mult(0.5);
  let yAxis = createVector(0, 1, 0);
  let cosTheta = constrain(yAxis.dot(dir), -1, 1);
  let angle = acos(cosTheta);
  let axis = p5.Vector.cross(yAxis, dir);
...
const TANGENT_DAMP = 0.997;
let BASEDIR_ROTATION_COUPLING = 0.012;
let BASEDIR_ROTATION_LERP = 0.12;
...
function formatAXnEmHTML(central, n, m) {
  ...
  if (n > 0) parts.push("X" + (n > 1 ? `<sub>${n}</sub>` : ""));
  if (m > 0) parts.push("E" + (m > 1 ? `<sub>${m}</sub>` : ""));
...
function restoreAngleAndAutoToInitial() {
  ...
  try {
    if (angleToggleBtn)
      angleToggleBtn.html(
        showAngleOverlay ? I18N[currentLanguage].angle_on : I18N[currentLanguage].angle_off
      );
  } catch (e) {}
  try {
    if (autoRotateBtn)
      autoRotateBtn.html(
        autoRotate ? I18N[currentLanguage].auto_on : I18N[currentLanguage].auto_off
      );
  } catch (e) {}
}
...
const MOLECULES = {
  ...
  "SO₂": { central: "S", domains: [ {type:"bond",element:"O",order:2},{type:"bond",element:"O",order:2},{type:"lonepair"} ], angleOverrides:{BB:119.5} },
  "BeCl₂": { central: "Be", domains: [ {type:"bond",element:"Cl",order:1},{type:"bond",element:"Cl",order:1} ], angleOverrides:{BB:180.0} },
  "XeF₂": { central: "Xe", domains: [ {type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"lonepair"},{type:"lonepair"},{type:"lonepair"} ], angleOverrides:{"ax-ax":180.0,BB:180.0} },
  ...
  "ClF₃": { central: "Cl", domains: [ {type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"lonepair"},{type:"lonepair"} ], angleOverrides:{"ax-eq":87.5,"eq-eq":175.0} },
  "BrF₃": { central: "Br", domains: [ {type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"lonepair"},{type:"lonepair"} ], angleOverrides:{"ax-eq":86.2} },
  ...
  "SF₄": { central: "S", domains: [ {type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"bond",element:"F",order:1},{type:"lonepair"} ], angleOverrides:{"ax-ax":173.0,"ax-eq":87.8,"eq-eq":101.5} },
...
function computeUniqueFanRadiusProposed(baseRadius, seedValue, usedSet) {
  ...
  slots.sort((a, b) => Math.abs(a - base) - Math.abs(b - a) || b - a);

  const EPS = 1e-6;
  for (let cand of slots) {
    let ok = true;
    for (let u of usedSet) {
      if (Math.abs(cand - u) < SPACING - EPS) {
        ok = false;
        break;
      }
    }
  ...
  while (attempts < 200) {
    let conflict = false;
    for (let u of usedSet) {
      if (Math.abs(candFallback - u) < SPACING - EPS) {
        conflict = true;
        break;
      }
    }
...
function assignPresetPositionsToDomains(domains) {
  ...
  function assign(domainIndex, posIndex) {
    ...
    assigned[domainIndex] = positions[posIndex]
      .copy()
      .normalize()
      .mult(domains[domainIndex].type === "lonepair" ? LONE_PAIR_BOND_LENGTH : BOND_LENGTH);
    used[posIndex] = true;
    return true;
  }
...
class ElectronDomain {
  ...
  displaySphereAndLabel() {
    ...
    ambientMaterial(220);
    fill(255);
    sphere(BOND_SPHERE_RADIUS, SPHERE_DETAIL, SPHERE_DETAIL);
    pop();

    if (this.element && showObjectLabels) {
      drawLabelText(this.pos, this.element, isMobileDevice ? 16 : 24, BOND_SPHERE_RADIUS, color(150, 0, 0));
    }
  }
}
...
class Bond extends ElectronDomain {
  ...
  display() {
    ...
    let startAbs = CENTRAL_ATOM_POS;
    let endAbs = this.pos;

    let dir = safeNormalize(p5.Vector.sub(endAbs, startAbs), createVector(1, 0, 0));
    ...
    let radiusBase = 5.5;
    if (this.order === 1) {
      drawCylinderBetween(startAbs, endAbs, radiusBase, CYL_DETAIL);
      ...
      drawCylinderBetween(s1, e1, radiusBase * 0.75, CYL_DETAIL);
      drawCylinderBetween(s2, e2, radiusBase * 0.75, CYL_DETAIL);
    } else {
      let off = 13;
      drawCylinderBetween(startAbs, endAbs, radiusBase * 0.7, CYL_DETAIL);
      let s1 = p5.Vector.add(startAbs, p5.Vector.mult(offsetDir, off));
      let e1 = p5.Vector.add(endAbs, p5.Vector.mult(offsetDir, off));
      let s2 = p5.Vector.sub(startAbs, p5.Vector.mult(offsetDir, off));
      let e2 = p5.Vector.sub(endAbs, p5.Vector.mult(offsetDir, off));
      drawCylinderBetween(s1, e1, radiusBase * 0.55, CYL_DETAIL);
      drawCylinderBetween(s2, e2, radiusBase * 0.55, CYL_DETAIL);
    }
  }
}
...
class LonePair extends ElectronDomain {
  constructor(position) {
    super("lonepair", position);
  }
  displayOval() {
    let axisOut = safeNormalize(
      this.baseDir ? this.baseDir.copy() : p5.Vector.sub(this.pos, CENTRAL_ATOM_POS),
      createVector(0, 0, 1)
    );
    ...
    let ry = rx,
      rz = BOND_SPHERE_RADIUS * 1.6;
    noStroke();
    ambientMaterial(80, 200, 200, 100);
    fill(0, 200, 200, 80);
    ellipsoid(rx, ry, rz, OVAL_DETAIL_X, OVAL_DETAIL_Y);
    pop();

    if (arialFont && showObjectLabels) {
      drawLabelText(ovalPos, "E", isMobileDevice ? 16 : 22, rz, color(255, 204, 0));
    }
  }
  displayElectrons() {
    let axisOut = safeNormalize(
    ...
    let forwardOffset = 4;
    push();
    translate(-BOND_SPHERE_RADIUS * 0.45, 0, forwardOffset);
    sphere(r, SPHERE_DETAIL, SPHERE_DETAIL);
    pop();
    push();
    translate(BOND_SPHERE_RADIUS * 0.45, 0, forwardOffset);
    sphere(r, SPHERE_DETAIL, SPHERE_DETAIL);
...
class PresetDomain {
  ...
  displayPresetSphereAndLabel() {
    ...
    fill(230, 240, 250);
    sphere(BOND_SPHERE_RADIUS * 0.88, SPHERE_DETAIL, SPHERE_DETAIL);
    pop();

    if (this.element && arialFont && showObjectLabels) {
      drawLabelText(this.pos, this.element, isMobileDevice ? 16 : 24, BOND_SPHERE_RADIUS * 0.88, color(150, 0, 0));
    }
  }
  ...
  displayPresetElectrons(centerPos) {
    ...
    noStroke();
    fill(200, 190, 170);
    let r = BOND_SPHERE_RADIUS * (0.3 * 2 / 3);
    let forwardOffset = 2;
    push();
    translate(-BOND_SPHERE_RADIUS * 0.45, 0, forwardOffset);
    sphere(r, SPHERE_DETAIL, SPHERE_DETAIL);
  ...
  displayPresetBond(centerPos) {
    ...
    const startAbs = centerPos;
    const endAbs = this.pos;
    const dir = safeNormalize(p5.Vector.sub(endAbs, startAbs), createVector(1, 0, 0));
    let offsetDir = p5.Vector.cross(dir, createVector(0, 1, 0));
    if (offsetDir.mag() < 1e-6) offsetDir = p5.Vector.cross(dir, createVector(0, 0, 1));
    offsetDir.normalize();

    const radiusBase = 3.6;
    if (this.order === 1) {
      drawCylinderBetween(startAbs, endAbs, radiusBase, CYL_DETAIL);
    } else if (this.order === 2) {
      const off = 7;
      ...
      let s2 = p5.Vector.sub(startAbs, p5.Vector.mult(offsetDir, off));
      let e2 = p5.Vector.sub(endAbs, p5.Vector.mult(offsetDir, off));
      drawCylinderBetween(s1, e1, radiusBase * 0.78, CYL_DETAIL);
      drawCylinderBetween(s2, e2, radiusBase * 0.78, CYL_DETAIL);
    } else {
      const off = 10;
      drawCylinderBetween(startAbs, endAbs, radiusBase * 0.72, CYL_DETAIL);
      let s1 = p5.Vector.add(startAbs, p5.Vector.mult(offsetDir, off));
      let e1 = p5.Vector.add(endAbs, p5.Vector.mult(offsetDir, off));
      let s2 = p5.Vector.sub(startAbs, p5.Vector.mult(offsetDir, off));
      let e2 = p5.Vector.sub(endAbs, p5.Vector.mult(offsetDir, off));
      drawCylinderBetween(s1, e1, radiusBase * 0.6, CYL_DETAIL);
      drawCylinderBetween(s2, e2, radiusBase * 0.6, CYL_DETAIL);
    }
  }
}

/////////////////////// Preset creation (small tilts) ///////////////////////
function createPresetMoleculeVisual(name) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    tempDomains.push({
      ...
      order: d.order || 1,
      pos: createVector(BOND_LENGTH, 0, 0),
    });
  }
  ...
  function getLpDirAverage(visual) {
    ...
    if (sum.mag() < 1e-6) {
      for (let i = 0; i < visual.domains.length; i++) {
        if (visual.domains[i].type === "lonepair")
          return safeNormalize(p5.Vector.sub(visual.domains[i].pos, CENTRAL_ATOM_POS));
      }
      return null;
    }
...
function drawPresetMolecule() {
  ...
  fill(80, 160, 200);
  sphere(CENTRAL_ATOM_RADIUS * centralScale * 1.02, SPHERE_DETAIL, SPHERE_DETAIL);
  pop();

  if (arialFont && showObjectLabels) {
    drawLabelText(CENTRAL_ATOM_POS, presetMoleculeVisual.central, isMobileDevice ? 16 : 24, CENTRAL_ATOM_RADIUS, color(255, 255, 0));
  }
...
function worldToScreen(worldPos) {
  ...
  let rectW =
    p5Canvas && p5Canvas.elt && p5Canvas.elt.width
      ? p5Canvas.elt.width
      : typeof width !== "undefined"
      ? width
      : window.innerWidth;
  let rectH =
    p5Canvas && p5Canvas.elt && p5Canvas.elt.height
      ? p5Canvas.elt.height
...
function drawAngleOverlaysForPreset() {
  ...
  if (presetMoleculeVisual.name === "SF₆") {
    ...
    for (let a = 0; a < bonds.length; a++) {
      for (let b = a + 1; b < bonds.length; b++) {
        ...
        let dirB = safeNormalize(B.pos ? p5.Vector.sub(B.pos, CENTRAL_ATOM_POS) : createVector(1, 0, 0));
      ...
      for (let rIdx = 0; rIdx < reps.length; rIdx++) {
        ...
        if (arialFont && pts.length > 0 && RENDER_ANGLE_LABELS) {
          ...
          if (mid) {
            ...
            // Orange angle labels
            drawLabelText(midPos, degLabel.toFixed(1) + "°", 20, 0, color(255, 165, 0));
          }
        }
      }
    }
  ...
  if (presetMoleculeVisual.name === "BrF₅") {
    ...
    bondInfos.forEach((b) => {
      b.proj = Math.abs(constrain(b.dir.dot(lpDir), -1, 1));
    });
  ...
  for (let a = 0; a < bonds.length; a++) {
    for (let b = a + 1; b < bonds.length; b++) {
      ...
      pairs.push({
        ...
        angle: angleRad,
        angleDeg,
        rolePair: [nrm(ri), nrm(rj)].sort().join("-"),
        chordDist,
      });
    }
  }
  ...
  for (let rIdx = 0; rIdx < reps.length; rIdx++) {
    ...
    let radius = computeUniqueFanRadiusProposed(
      proposedBase,
      rep.chordDist + rep.angleDeg * 1.7 + rIdx * 2.3,
      usedRadii
    );
...
function drawAngleOverlaysForSimulation() {
  ...
  for (let [k, arr] of groups.entries()) {
    ...
    for (let i = 1; i < arr.length; i++) {
      if ((arr[i].chordDist || 0) > bestDist) {
        best = arr[i];
        bestDist = arr[i].chordDist || 0;
      }
    }
    reps.push(best);
  }
  ...
  if (snap) {
    for (let i = 0; i < electronDomains.length; i++) {
      ...
      if (assigned[i]) {
        ...
        dom.pos = p5.Vector.add(
          CENTRAL_ATOM_POS,
          p5.Vector.mult(dom.baseDir, dom.type === "lonepair" ? LONE_PAIR_BOND_LENGTH : BOND_LENGTH)
        );
...
function startSyncToAssignedTargets(durationOverride) {
  ...
  for (let i = 0; i < n; i++) {
    let dom = electronDomains[i];
    syncInitialDirs[i] = dom.baseDir
      ? dom.baseDir.copy()
      : safeNormalize(p5.Vector.sub(dom.pos, CENTRAL_ATOM_POS), createVector(1, 0, 0));
    if (targetsByDomain[i])
      syncTargetDirs[i] = safeNormalize(p5.Vector.sub(targetsByDomain[i], CENTRAL_ATOM_POS), syncInitialDirs[i]);
...
/////////////////////// Representative fan computation for preset ///////////////////////
function computeRepresentativeFanDirectionForPreset() {
  ...
  if (presetMoleculeVisual.name === "IF₇" && Math.abs(Number(chosenKey) - 72.0) < 0.5) {
    ...
    for (let p of arr) {
      if (p.chordDist < bestDist) {
        rep = p;
        bestDist = p.chordDist;
      }
    }
  } else {
    let bestDist = arr[0].chordDist;
...
function computeRotationForDirection(targetDir) {
  ...
  sceneRotX = wrapAngle(sceneRotX);
  sceneRotY = wrapAngle(sceneRotY);
...
const ADDITION_SYNC_DURATION = 1100;
...
function endDragAtClientCoords(clientX, clientY) {
  ...
  if (draggedElementType === "lonepair") {
    ...
    if (dist <= threshold) {
      ...
      let radius = dir.mag();
      let minR = CENTRAL_ATOM_RADIUS + 8;
      if (radius < minR) radius = minR;
      dir.normalize();
      pos = p5.Vector.add(CENTRAL_ATOM_POS, p5.Vector.mult(dir, radius));
    }
    let newDom = new LonePair(pos);
    newDom.tangentOffset = createVector(0, 0, 0);
    newDom.swayStartTime = millis();
    newDom.swayDuration = random(300, 900);
    newDom.spawnTime = millis();
    electronDomains.push(newDom);

    restoreAngleAndAutoToInitial();

    applyVSEPRPositions(ADDITION_SYNC_DURATION, false);
  } else {
    let order = 1;
...
/////////////////////// Soft, distance-weighted repulsion with spawn ramp ///////////////////////
function computeSoftRepulsion(dist, pairCharge, repFactor) {
  const MIN_DIST = 40;
  const FADE_DIST = 460;
  let clamped = max(dist, 1.0);
  let base = (BASE_REPULSION_CONSTANT * repFactor * pairCharge) / (clamped * clamped);
  let t = constrain((clamped - MIN_DIST) / (FADE_DIST - MIN_DIST), 0, 1);
  let falloff = pow(1 - t, 1.4);
  return base * falloff;
}
function settleFactor(dom, nowMs) {
  if (!dom || !dom.spawnTime) return 1;
  let dt = max(0, nowMs - dom.spawnTime);
  return constrain(dt / 900, 0, 1);
}
...
function simulateRepulsion() {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      ...
      let rampA = settleFactor(A, now);
      let rampB = settleFactor(B, now);
      let ramp = sqrt(rampA * rampB);

      let fmag =
        computeSoftRepulsion(dist, pairCharge, repFactor) *
        (repulsionConstant / BASE_REPULSION_CONSTANT) *
        ramp;
  ...
  for (let idx = 0; idx < n; idx++) {
    ...
    let env = 1.0;
    if (dom.swayDuration && dom.swayDuration > 0) {
      env = constrain(1 - age / dom.swayDuration, 0, 1);
      env = pow(env, 1.5);
    }
    ...
    if (isVSEPRMode && dom.vseprActive && dom.vseprDir) {
      ...
      if (angleError > 0.001) {
        ...
        let osc =
          sin((now * 0.001) * dom.vibFreq + dom.vibPhase) *
          dom.vibAmp *
          0.08 *
          (angleError / PI);
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        let fmag =
          computeSoftRepulsion(dist, pairCharge, repFactor) *
          (relaxConst / BASE_REPULSION_CONSTANT) *
          ramp;
        fmag = min(fmag, 8000);
        let fvec = safeMult(delta.normalize(), fmag);
        let dirA = safeNormalize(p5.Vector.sub(A.pos, CENTRAL_ATOM_POS), createVector(1, 0, 0));
...
/////////////////////// UI ///////////////////////
function createUI() {
  const leftHost = select("#left-panel");
  ...
  brandBox.addClass("sidebar-brand");
  topCenterLabel = createDiv(I18N[currentLanguage].topLabel);
  topCenterLabel.addClass("sidebar-brand-title");
  topCenterLabel.parent(brandBox);
  bottomCenterLabel = createDiv("© HÓA HỌC ABC");
  bottomCenterLabel.addClass("sidebar-brand-subtitle");
  bottomCenterLabel.parent(brandBox);

  let leftUI = createDiv();
  leftUI.parent(leftHost);
  leftUI.addClass("panel-content");

  const modernControlBase = (el) => {
    el.addClass("control-base");
  };
  ...
  modernControlBase(languageSelect);
  languageSelect.addClass("select-control");
  ...
  try {
    moleculeSelect.elt.options[0].disabled = true;
    moleculeSelect.elt.selectedIndex = 0;
  } catch (e) {}
  modernControlBase(moleculeSelect);
  moleculeSelect.addClass("select-control");
  ...
  modernControlBase(angleToggleBtn);
  angleToggleBtn.addClass("control-button");
  ...
  autoRotateBtn = createButton(
    autoRotate ? I18N[currentLanguage].auto_on : I18N[currentLanguage].auto_off
  );
  autoRotateBtn.parent(leftUI);
  modernControlBase(autoRotateBtn);
  autoRotateBtn.addClass("control-button");
  ...
  labelToggleBtn = createButton(
    showObjectLabels ? I18N[currentLanguage].labels_on : I18N[currentLanguage].labels_off
  );
  labelToggleBtn.parent(leftUI);
  modernControlBase(labelToggleBtn);
  labelToggleBtn.addClass("control-button");
  labelToggleBtn.mousePressed(() => {
  ...
  screenshotBtn = createButton(I18N[currentLanguage].screenshot);
  screenshotBtn.parent(leftUI);
  modernControlBase(screenshotBtn);
  screenshotBtn.addClass("control-button");
  ...
  modernControlBase(resetBtn);
  resetBtn.addClass("control-button");
  ...
  modernControlBase(notesBtn);
  notesBtn.addClass("control-button");
  notesBtn.mousePressed(() => {
    toggleNotesModal();
  });

  formulaDiv = createDiv("AXnEm");
  formulaDiv.parent(leftUI);
  formulaDiv.addClass("formula");

  let rightUI = createDiv();
  rightUI.parent(rightHost);
  rightUI.addClass("panel-content");
  ...
  notesContentDiv.addClass("notes-content");

  notesModal.notesContentDiv = notesContentDiv;
  notesModal.modalBox = modalBox;

  document.body.appendChild(notesModal.elt);
  ...
  // Sidebar hover/touch to disable canvas interactions
  const markSidebarTouch = (active) => {
    pointerOverSidebar = active;
  };
  [leftHost, rightHost].forEach((host) => {
    if (!host || !host.elt) return;
    host.elt.addEventListener("mouseenter", () => {
      pointerOverSidebar = true;
    });
    host.elt.addEventListener("mouseleave", () => {
      pointerOverSidebar = false;
    });
    host.elt.addEventListener("touchstart", () => markSidebarTouch(true), { passive: true });
    host.elt.addEventListener("touchend", () => markSidebarTouch(false), { passive: true });
    host.elt.addEventListener("touchcancel", () => markSidebarTouch(false), { passive: true });
  });

  // Allow right-drag rotation without context menu
  if (p5Canvas && p5Canvas.elt) {
...
function toggleNotesModal() {
  ...
  if (notesModal.elt.classList.contains("visible")) hideNotesModal();
  else showNotesModal();
}
function showNotesModal() {
  if (!notesModal) return;
  notesModal.addClass("visible");
  if (notesModal.notesContentDiv) notesModal.notesContentDiv.html(I18N[currentLanguage].notesContent);
}
function hideNotesModal() {
  if (!notesModal) return;
  notesModal.removeClass("visible");
}
...
function mouseDragged(event) {
  ...
  // Shift-key or right-button drag -> roll (rotationZ), else orbit (rotationX/Y)
  const isRollDrag = (event && event.shiftKey) || (event && event.buttons === 2);
  if (isRollDrag) {
    rotationZ = wrapAngle(rotationZ - dx * 0.01);
  } else {
    rotationY -= dx * 0.01;
    rotationX -= dy * 0.01;
  }
...
function updatePreviewDomain() {
  ...
  let dir = previewDomain.target
    ? p5.Vector.sub(previewDomain.target, CENTRAL_ATOM_POS)
    : p5.Vector.sub(previewDomain.pos, CENTRAL_ATOM_POS);
  ...
  let env = 1.0;
  if (previewDomain.swayDuration && previewDomain.swayDuration > 0) {
    env = constrain(1 - age / previewDomain.swayDuration, 0, 1);
    env = pow(env, 1.5);
  }
  let tnow = now * 0.001;
...
function updateDomainListUI() {
  ...
  electronDomains.forEach((dom) => {
    let row = createDiv();
    row.parent(domainListPanel);
    row.addClass("domain-row");
    let left = createDiv();
    left.parent(row);
    left.addClass("domain-left");
    let iconHtml = "";
    if (dom.type === "bond") {
      if (dom.order === 2) {
        iconHtml = `
    ...
    let icon = createDiv(iconHtml);
    icon.parent(left);
    icon.elt.style.pointerEvents = "none";
    let label = createDiv(
      `#${dom.id} (${dom.type}${dom.element ? " " + dom.element : ""}${dom.order ? " x" + dom.order : ""})`
    );
    label.parent(left);
    label.addClass("domain-label");
    ...
    if (selectedMolecule) {
      ...
      removeBtn.mousePressed(() => {
        removeDomainById(dom.id);
        try {
          if (moleculeSelect && moleculeSelect.elt) moleculeSelect.elt.selectedIndex = 0;
        } catch (e) {}
      });
    }
  });
...
function removeDomainById(id) {
  const nid = Number(id);
  if (isNaN(nid)) return;
  if (selectedMolecule) return;
  let index = electronDomains.findIndex((dom) => dom.id === nid);
  if (index !== -1) {
    electronDomains.splice(index, 1);
    restoreAngleAndAutoToInitial();
...
function updateUILanguage() {
  ...
  if (labelToggleBtn)
    labelToggleBtn.html(showObjectLabels ? I18N[currentLanguage].labels_on : I18N[currentLanguage].labels_off);
  if (screenshotBtn) screenshotBtn.html(I18N[currentLanguage].screenshot);
  ...
  if (notesModal && notesModal.elt.classList.contains("visible")) {
    if (notesModal.notesContentDiv) notesModal.notesContentDiv.html(I18N[currentLanguage].notesContent);
  }
  updateFormulaFromState();
}
...
function drawCentralLabel() {
  ...
  if (!arialFont || !showObjectLabels) return;
  drawLabelText(CENTRAL_ATOM_POS, label, isMobileDevice ? 16 : 24, CENTRAL_ATOM_RADIUS, color(255, 255, 0));
}

/* ========== applyMolecule & clearSelectedMolecule ========== */
function applyMolecule(name) {
  ...
  if (!savedElectronDomains) {
    savedElectronDomains = electronDomains;
    electronDomains = [];
  } else {
    savedElectronDomains = electronDomains;
    electronDomains = [];
...
function clearSelectedMolecule() {
  ...
  isVSEPRMode = false;
  if (savedElectronDomains) {
    electronDomains = savedElectronDomains;
    savedElectronDomains = null;
  }
  presetMoleculeVisual = null;
  if (presetPreviousShowAngleOverlay !== null) {
    showAngleOverlay = presetPreviousShowAngleOverlay;
    presetPreviousShowAngleOverlay = null;
  }
  if (presetPreviousAutoRotate !== null) {
    autoRotate = presetPreviousAutoRotate;
    presetPreviousAutoRotate = null;
  }
  setDragButtonsEnabled(true);
  try {
...
function requestScreenshot4K() {
  if (captureRequested || captureActive) return;
  captureRequested = true;
}

function beginCaptureIfRequested() {
  if (!captureRequested || captureActive) return;
...
function endCapture() {
  ...
  saveCanvas(p5Canvas, "vsepr-4k", "png");
  if (captureOriginal) {
    pixelDensity(captureOriginal.px || 1);
    resizeCanvas(captureOriginal.w, captureOriginal.h);
    perspective(PI / 4, width / max(1, height), 0.1, 5000);
  }
  captureActive = false;
}
...
function draw() {
  if (captureRequested && !captureActive) {
    beginCaptureIfRequested();
  }

  zoom = lerp(zoom, targetZoom, ZOOM_LERP);
  if (captureActive) clear();
  else background(0);

  const lightScale = captureActive ? 0.8 : 1.0;
  applySceneLights(lightScale);

  if (orientActive) {
    let elapsed = millis() - orientStartTime;
    let t = constrain(elapsed / max(1, orientDuration), 0, 1);
    let eased = easeInOutCubic(t);
    let sx = orientStartRotX,
      tx = orientTargetRotX;
    let dx = tx - sx;
    if (dx > PI) dx -= TWO_PI;
    if (dx < -PI) dx += TWO_PI;
    rotationX = sx + dx * eased;
    let sy = orientStartRotY,
      ty = orientTargetRotY;
    let dy = ty - sy;
    if (dy > PI) dy -= TWO_PI;
    if (dy < -PI) dy += TWO_PI;
    rotationY = sy + dy * eased;
    let sz = orientStartRotZ,
      tz = orientTargetRotZ;
    let dz = tz - sz;
    if (dz > PI) dz -= TWO_PI;
    if (dz < -PI) dz += TWO_PI;
  ...
  // keep angles bounded for stable 360° interaction
  rotationX = wrapAngle(rotationX);
  rotationY = wrapAngle(rotationY);
  rotationZ = wrapAngle(rotationZ);

  if (!orientActive && autoRotateSuspendedDuringOrient) {
    autoRotate = true;
    autoRotateSuspendedDuringOrient = false;
  }
  ...
  if (isSyncing) {
    ...
    for (let i = 0; i < electronDomains.length; i++) {
      let dom = electronDomains[i];
      let initDir =
        syncInitialDirs[i] && syncInitialDirs[i].copy
          ? syncInitialDirs[i].copy()
          : dom.baseDir
          ? dom.baseDir.copy()
          : safeNormalize(p5.Vector.sub(dom.pos, CENTRAL_ATOM_POS), createVector(1, 0, 0));
      let tgtDir =
        syncTargetDirs[i] && syncTargetDirs[i].copy
          ? syncTargetDirs[i].copy()
          : initDir.copy();
  ...
  if (!presetMoleculeVisual) {
    push();
    ambientMaterial(200, 40, 40);
    fill(200, 40, 40);
    sphere(CENTRAL_ATOM_RADIUS * centralScale, SPHERE_DETAIL, SPHERE_DETAIL);
    pop();
    drawCentralLabel();
  }
  ...
  if (isDragging && ghostWorld && !previewDomain) {
    push();
    translate(ghostWorld.x, ghostWorld.y, ghostWorld.z);
    noStroke();
    fill(255, 255, 255, 200);
    sphere(BOND_SPHERE_RADIUS * (draggedElementType === "lonepair" ? 1.18 : 1.0), SPHERE_DETAIL, SPHERE_DETAIL);
    pop();
  }
  ...
  if (previewDomain) {
    updatePreviewDomain();
    if (previewDomain) {
      let reveal = previewDomain.bondReveal !== undefined && previewDomain.bondReveal !== null ? previewDomain.bondReveal : 0;
      if (reveal > 0.01) {
        let bondEndPoint = previewDomain.pos.copy();
        drawCylinderBetween(CENTRAL_ATOM_POS, bondEndPoint, 5 * reveal + 2, CYL_DETAIL);
        ...
        if (previewDomain.type === "lonepair") {
          ...
          let startAbs = CENTRAL_ATOM_POS;
          let endAbs = previewDomain.pos;
          drawCylinderBetween(startAbs, endAbs, 4.5, CYL_DETAIL);
